...
- (id) initWithConsumerKey:(NSString *)aConsumerKey andConsumerSecret:(NSString *)aConsumerSecret {
	if ((self = [super init])) {
		oauth_consumer_key = [aConsumerKey retain];
		oauth_consumer_secret = [aConsumerSecret retain];
		oauth_signature_method = @"HMAC-SHA1";
		oauth_version = @"1.0";
		self.oauth_token = @"";
		self.oauth_token_secret = @"";
		srandom(arc4random()); // seed the random number generator, used for generating nonces
...
- (NSString *)signClearText:(NSString *)text withSecret:(NSString *)secret {
    NSData *signature = HMAC_SHA1(text,secret);
...
- (NSString *) oAuthHeaderForMethod:(NSString *)method
	...
	NSString *oauth_signature = [self signClearText:[self oauth_signature_base:method
																  withUrl:url
																andParams:_params]
...
- (NSString *) oauth_signature_base:(NSString *)httpMethod withUrl:(NSString *)url andParams:(NSDictionary *)params {
	...
	oauth_timestamp = [NSString stringWithFormat:@"%ld", timestamp];
	long myRandom = random();
	oauth_nonce = [self sha1:[NSString stringWithFormat:@"%ld%ld", timestamp, myRandom]];
...
- (NSString *)sha1:(NSString *)str {
	...
	u_int8_t result[CC_SHA1_DIGEST_LENGTH];
